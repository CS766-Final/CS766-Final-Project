...
These tables show that at the upper bounds of their capability to reconstruct detail, the pixel-wise methods performed better overall than the CNN. Detectron detected the same objects with a higher confidence on HSV and LCh reconstructed images than it did on the original saturated frame. In addition, we see the failure cases of the CNN cause the original saturated images to yield better detection scores. Going further, we see that in the extreme case of a 3 EV gain, the HSV reconstruction gains a significant majority over the original saturated image. This indicates that it has merit in being used prior to downstream computer vision tasks involving detection.

## Conclusions

The downstream applications, such as object detection, see some benefit from the usage of HDR reconstruction. The pixel-wise methods are robust, portable, and efficient. However, because of the failure cases, these methods could be further improved from being combined with inpainting or another region-based techniques. On our hardware, HDR CNNs are not able to provide a quick enough reconstruction for it to be considered realtime. Although, it is possible that with dedicated hardware it could run more efficiently.  It is also worth mentioning that single-frame HDR CNNs are still in a relative infancy, most trained on preprocessed images, and thus new CNNs and training strategies could change this result in the future.
