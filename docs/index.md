...
We did not modify the CNN or create an optimized implementation as the reference implementation is written to use CUDA instructions via Tensorflow. The HDRCNN took roughly 300ms per image, translating to roughly 3 fps on 1K 4:3 images. This is markedly slower than the pixel-wise methods. However, the drop in performance might be reconciled in the evaluation of the final recovery.
